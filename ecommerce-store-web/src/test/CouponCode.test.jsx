...
describe('CouponCode Component', () => {
  ...
  it('renders input field and apply button', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByRole('button', { name: /Apply code/i })).toBeInTheDocument();
    });
  });
  ...
  it('displays message when no coupons are available', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/No coupon codes available. Place more 5 orders to get a coupon code./i)
      ).toBeInTheDocument();
    });
  });
  ...
  it('calculates correct number of remaining orders needed', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/No coupon codes available. Place more 2 orders to get a coupon code./i)).toBeInTheDocument();
    });
  });
